<template>
  ...
  >
    ...
    <img
      ...
      alt=""
      class="pointer-events-none absolute bottom-0 left-0 w-40 opacity-15 rotate-180"
    />
    ...
    <div class="relative mx-auto mt-6 w-full max-w-md">
      ...
      <svg
        v-else
        class="pointer-events-none absolute right-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[#1E1E1E]/35"
        viewBox="0 0 20 20"
        fill="currentColor"
      >
        <path
          fill-rule="evenodd"
          d="M12.9 14.32a8 8 0 111.414-1.414l4.387 4.387-1.414 1.414-4.387-4.387zM8 14A6 6 0 108 2a6 6 0 000 12z"
          clip-rule="evenodd"
        />
      </svg>
    </div>
  ...
  <div v-if="initialLoading">
    <div
      ...
      :key="n"
      class="animate-pulse"
      :class="n % 2 === 0 ? 'bg-white' : 'bg-[#FDE8F2]'"
    >
      <div class="mx-auto max-w-4xl px-6 py-6 flex gap-6 items-center">
        <div
          class="flex-shrink-0 w-36 h-36 sm:w-44 sm:h-44 rounded-2xl bg-black/10"
        ></div>
  ...
  <template v-else>
    ...
    >
      ...
      >
        <!-- Image -->
        <div
          class="flex-shrink-0 w-36 h-36 sm:w-44 sm:h-44 rounded-2xl overflow-hidden bg-gradient-to-br from-[#F7C0DB] to-[#F48AC1] shadow-sm"
        >
          <img
        ...
        <div class="flex-1 min-w-0 flex flex-col gap-2">
          <span
            ...
            :class="
              index % 2 === 0
                ? 'bg-white/70 text-[#1E1E1E]/50'
                : 'bg-[#FDE8F2] text-[#1E1E1E]/50'
            "
          >
            {{
          ...
          <h2
            class="font-serif text-xl sm:text-2xl text-[#1E1E1E] leading-snug"
          >
            {{ capitalize(product.name) }}
          </h2>
          <p
            v-if="product.description"
            class="text-[#1E1E1E]/60 text-sm leading-relaxed line-clamp-3"
          >
            {{ product.description }}
          </p>
        </div>
    ...
    <div v-if="products.length === 0" class="py-24 text-center">
      <p class="text-[#1E1E1E]/40 text-sm">
        <template v-if="searchQuery"
          >No se encontraron productos para “{{ searchQuery }}”</template
        >
    ...
    <div class="flex flex-col items-center gap-3 py-8 bg-white">
      <p class="text-[#1E1E1E]/40 text-xs">
      ...
      <p v-else-if="products.length > 0" class="text-[#1E1E1E]/35 text-xs">
        Eso es todo — has visto todos los productos
      </p>
    </div>
  </template>
</template>
