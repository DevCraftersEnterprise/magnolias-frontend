...
function formatMexPhone(raw: string | null | undefined): string {
    if (!raw) return ''
...
<template>
    <div class="relative bg-white rounded-2xl shadow p-5 min-w-[240px] max-w-xs transition-opacity" :class="!isActive ? 'opacity-60' : ''">
        <div class="flex items-center justify-between mb-1">
            <span class="font-bold text-lg text-[#1E1E1E]">{{ name }}</span>
            <button class="text-gray-300 hover:text-pink-400 transition-colors" type="button" @click="emit('edit')">
        ...
        <div class="mb-3">
            <span v-if="!isActive" class="inline-flex items-center gap-1 rounded-full bg-black/8 px-2.5 py-0.5 text-[10px] font-semibold text-black/50">
        ...
        <div class="flex items-center gap-2 text-[#6B7280] mb-2">
            <div class="flex items-center gap-2 text-[#6B7280] mb-2">
                <svg xmlns="http://www.w3.org/2000/svg" fill="#f472b6" class="w-5 h-5 flex-shrink-0"
                    viewBox="0 0 24 24">
                    <path
        ...
        <div class="flex items-center gap-2 text-[#6B7280] mb-1">
            <div class="flex items-center gap-2 text-[#6B7280] mb-2">
                <svg xmlns="http://www.w3.org/2000/svg" fill="#f472b6" class="w-5 h-5" viewBox="0 0 24 24">
                    <path
                        d="M20.01 15.38c-1.23 0-2.42-.2-3.53-.56-.35-.12-.74-.03-1.01.24l-1.57 1.97c-2.83-1.35-5.48-3.9-6.89-6.83l1.95-1.66c.27-.28.35-.67.24-1.02-.37-1.11-.56-2.3-.56-3.53 0-.54-.45-.99-.99-.99H4.19C3.65 3 3 3.24 3 3.99 3 13.28 10.73 21 20.01 21c.71 0 .99-.63.99-1.18v-3.45c0-.54-.45-.99-.99-.99z" />
                </svg>
                <span class="text-sm">{{ formatMexPhone(phone1) }}</span>
            </div>
        </div>
        <div v-if="phone2" class="flex items-center gap-2 text-[#6B7280] mb-1">
            <div class="flex items-center gap-2 text-[#6B7280] mb-2">
                ...
                <span class="text-sm">{{ formatMexPhone(phone2) }}</span>
            </div>
        </div>
