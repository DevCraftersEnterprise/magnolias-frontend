    ...
    <form @submit.prevent="onSubmit" class="space-y-4">
      ...
      <div class="grid grid-cols-2 gap-3">
        ...
        <div>
          <label class="text-xs font-semibold text-black/60">
            ...
            <span v-if="mode === 'edit'" class="font-normal text-black/40">(opcional)</span>
          </label>
          <input
      ...
      <template v-else-if="form.role === 'BAKER'">
        <div>
          <label class="text-xs font-semibold text-black/60">
            Sucursales
            <span class="font-normal text-black/40">(puedes elegir varias)</span>
          </label>
          <div class="mt-1 rounded-xl bg-black/5 p-3 space-y-2 max-h-40 overflow-y-auto">
            <label
              v-for="b in branches"
              :key="b.id"
            ...
            <p v-if="branches.length === 0" class="text-xs text-black/40">Cargando sucursales…</p>
          </div>
        </div>
        <div>
      ...
      <div v-if="mode === 'edit'" class="flex items-center justify-between rounded-xl bg-black/5 px-4 py-3">
...
import { usersService, type UserItem, type CreateUserPayload, type UpdateUserPayload } from '~/services/users.service'
...
onMounted(async () => {
  ...
  } catch { /* si falla, los selects quedarán vacíos */ }
})
...
async function onSubmit() {
  ...
  try {
    ...
    if (props.mode === 'create') {
      ...
      if (form.userkey) editPayload.userkey = form.userkey
      if (payload.branchId !== undefined)  editPayload.branchId  = payload.branchId
      if (payload.branchIds !== undefined) editPayload.branchIds = payload.branchIds
      if (payload.area !== undefined)      editPayload.area      = payload.area
      if (payload.specialty !== undefined) editPayload.specialty = payload.specialty

      if (form.isActive) {
        await usersService.updateUser(editPayload)
      } else {
        await usersService.deleteUser(editPayload)
      }
      emit('saved')
    }
