...
function onMousemove(e: MouseEvent) {
  ...
  pan.value = { x: panStart.value.x + e.clientX - dragStart.value.x, y: panStart.value.y + e.clientY - dragStart.value.y }
}
...
<template>
  <section class="min-h-[calc(100vh-86px)] bg-[#F3F3F4] font-sans">
    <div class="mx-auto w-full max-w-[1260px] px-4 py-6 lg:px-8 lg:py-8">
      ...
      <template v-else-if="order">
        ...
        <!-- ══ HEADER CARD ═══════════════════════════════════════════════════ -->
        <div class="rounded-2xl bg-white ring-1 ring-black/10 shadow-[0_10px_28px_rgba(16,24,40,0.06)] px-6 py-5 mb-5 flex flex-wrap items-center justify-between gap-4">
          <div class="min-w-0">
            <p class="text-[11px] font-semibold tracking-widest text-gray-400 uppercase">Pedido</p>
            <div class="flex flex-wrap items-center gap-2.5 mt-0.5">
              <h1 class="text-[22px] font-bold text-[#111827] tracking-tight leading-tight">{{ order.orderCode }}</h1>
              <span
              ...
              <span v-if="order.isCustomerPickup" class="inline-flex rounded-full bg-amber-50 px-3 py-1 text-[12px] font-semibold text-amber-700 ring-1 ring-amber-200/50">
                Recoge en tienda
              </span>
              <span v-if="order.branch" class="inline-flex items-center gap-1.5 rounded-full bg-gray-100 px-3 py-1 text-[12px] text-gray-600">
                <svg class="h-3 w-3 text-gray-400 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
                {{ order.branch.name }}
              </span>
            </div>
          </div>
          ...
          >
            <svg v-if="advancing" class="h-4 w-4 animate-spin" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>
            {{ advancing ? 'Actualizando…' : advanceLabel }}
          </button>
          <span v-else-if="order.status === 'DONE'" class="inline-flex items-center gap-1.5 rounded-xl bg-[#B9D9FF] px-4 py-2.5 text-[13px] font-bold text-[#0047C9]">
        ...
        <!-- Error de avance -->
        <div v-if="advanceError" class="mb-4 rounded-xl bg-red-50 px-4 py-3 text-[13px] text-red-700 ring-1 ring-red-200">
          {{ advanceError }}
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 items-start">
          ...
          <div class="lg:col-span-2 space-y-5">
            ...
            <div v-if="order.details && order.details.length > 0">
              <div class="flex items-center gap-2 mb-3">
                ...
                <span class="rounded-full bg-[#F5E6FA] px-2.5 py-0.5 text-[11px] font-bold text-[#7C00C9]">{{ order.details.length }}</span>
              </div>
              ...
              <div class="space-y-4">
                ...
                >
                  ...
                  >
                    <img :src="detail.referenceImageUrl" alt="Referencia" class="w-full max-h-80 object-cover bg-gray-100 transition group-hover:brightness-90" />
                    <span class="absolute bottom-3 right-3 rounded-lg bg-black/50 px-2.5 py-1 text-[11px] font-semibold text-white backdrop-blur-sm flex items-center gap-1.5">
                      <svg class="h-3 w-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35M11 8v6M8 11h6"/></svg>
                      Ver foto
                    </span>
                  </div>

                  <!-- Nombre + cantidad -->
                  <div class="px-6 pt-5 pb-4 flex items-start justify-between gap-4 border-b border-black/[0.06]">
                    <div class="min-w-0">
                      <p class="text-[20px] font-bold text-[#111827] leading-tight">{{ detail.product?.name ?? `Producto ${i + 1}` }}</p>
                      <p v-if="detail.product?.description" class="mt-1 text-[13px] text-gray-400">{{ detail.product.description }}</p>
                    </div>
                    <span class="shrink-0 rounded-xl bg-[#F5E6FA] px-4 py-2 text-[16px] font-bold text-[#7C00C9]">× {{ detail.quantity }}</span>
                  </div>
                  ...
                  <dl class="divide-y divide-black/[0.06]">
                    ...
                    <div v-if="detail.flavor" class="flex items-center justify-between px-6 py-3.5">
                      ...
                      <dd class="text-[15px] font-bold text-[#111827] text-right">{{ detail.flavor.name }}</dd>
                    </div>
                    <div v-if="detail.filling" class="flex items-center justify-between px-6 py-3.5">
                      <dt class="text-[11px] font-semibold uppercase tracking-widest text-gray-400 shrink-0 mr-6">Relleno</dt>
                      <dd class="text-[15px] font-bold text-[#111827] text-right">{{ detail.filling.name }}</dd>
                    </div>
                    <div v-if="detail.frosting" class="flex items-center justify-between px-6 py-3.5">
                      <dt class="text-[11px] font-semibold uppercase tracking-widest text-gray-400 shrink-0 mr-6">Betún</dt>
                      <dd class="text-[15px] font-bold text-[#111827] text-right">{{ detail.frosting.name }}</dd>
                    </div>
                    <div v-if="detail.breadType" class="flex items-center justify-between px-6 py-3.5">
                      <dt class="text-[11px] font-semibold uppercase tracking-widest text-gray-400 shrink-0 mr-6">Tipo de pan</dt>
                      <dd class="text-[15px] font-bold text-[#111827] text-right">{{ detail.breadType.name }}</dd>
                    </div>
                    <div v-if="detail.color" class="flex items-center justify-between px-6 py-3.5">
                      <dt class="text-[11px] font-semibold uppercase tracking-widest text-gray-400 shrink-0 mr-6">Color</dt>
                      <dd class="text-[15px] font-bold text-[#111827] text-right">{{ detail.color.name }}</dd>
                    </div>
                    <div v-if="detail.style" class="flex items-center justify-between px-6 py-3.5">
                      <dt class="text-[11px] font-semibold uppercase tracking-widest text-gray-400 shrink-0 mr-6">Estilo</dt>
                      <dd class="text-[15px] font-bold text-[#111827] text-right">{{ detail.style.name }}</dd>
                    </div>
                  </dl>

                  <!-- Texto en pastel -->
                  <div v-if="detail.hasWriting" class="border-t border-black/[0.06]">
                    <!-- Cabecera con ícono de lápiz -->
                    <div class="flex items-center gap-2 px-6 pt-4 pb-3">
                      <svg class="h-4 w-4 text-[#C9007C] shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                      <span class="text-[11px] font-bold uppercase tracking-widest text-[#C9007C]">Texto en pastel</span>
                    </div>
                    <!-- Texto principal: grande, centrado, como si estuviera escrito en el pastel -->
                    <div class="mx-6 mb-4 rounded-2xl bg-[#FFBEE6]/20 ring-1 ring-[#FFBEE6] px-5 py-5 text-center">
                      <p v-if="detail.writingText" class="text-[22px] font-bold text-[#111827] italic leading-snug tracking-wide">"{{ detail.writingText }}"</p>
                      <p v-else class="text-[14px] text-[#C9007C]/50 italic">Sin texto especificado</p>
                      <!-- Detalles de ubicación debajo del texto, separados -->
                      <div v-if="detail.writingLocation || detail.pipingLocation" class="mt-3 flex justify-center flex-wrap gap-3 border-t border-[#FFBEE6] pt-3">
                        <span v-if="detail.writingLocation" class="inline-flex items-center gap-1.5 text-[12px] text-[#C9007C]">
                          <span class="font-semibold">Ubicación</span>
                          <span class="opacity-40">·</span>
                          {{ detail.writingLocation }}
                        </span>
                        <span v-if="detail.pipingLocation" class="inline-flex items-center gap-1.5 text-[12px] text-[#C9007C]">
                  ...
                  <!-- Instrucciones -->
                  <div v-if="detail.decorationNotes || detail.notes" class="border-t border-black/[0.06]">
                    <div class="flex items-center gap-2 px-6 pt-4 pb-3">
                      <svg class="h-4 w-4 text-gray-400 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
                      <span class="text-[11px] font-bold uppercase tracking-widest text-gray-400">Instrucciones especiales</span>
                    </div>
                    <div class="mx-6 mb-5 space-y-2">
                      <div v-if="detail.decorationNotes" class="rounded-xl bg-gray-50 ring-1 ring-black/[0.07] px-4 py-3">
                        <p class="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-1">Decoración</p>
                        <p class="text-[14px] text-[#111827] leading-relaxed">{{ detail.decorationNotes }}</p>
                      </div>
                      <div v-if="detail.notes" class="rounded-xl bg-gray-50 ring-1 ring-black/[0.07] px-4 py-3">
                        <p class="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-1">Nota</p>
                        <p class="text-[14px] text-[#111827] leading-relaxed">{{ detail.notes }}</p>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div v-if="order.orderFlowers && order.orderFlowers.length > 0">
              <div class="flex items-center gap-2 mb-3">
                ...
                <span class="rounded-full bg-[#F5E6FA] px-2.5 py-0.5 text-[11px] font-bold text-[#7C00C9]">{{ order.orderFlowers.length }}</span>
              </div>
              <div class="rounded-2xl bg-white ring-1 ring-black/10 shadow-[0_10px_28px_rgba(16,24,40,0.06)] overflow-hidden divide-y divide-black/[0.06]">
                <div v-for="(f, i) in order.orderFlowers" :key="i" class="px-6 py-4 flex items-center gap-4">
                  <span
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <p v-if="f.notes" class="mt-0.5 text-[12px] text-gray-400 italic">{{ f.notes }}</p>
                  </div>
                  <span class="shrink-0 rounded-xl bg-[#F5E6FA] px-4 py-2 text-[15px] font-bold text-[#7C00C9]">× {{ f.quantity }}</span>
                </div>
              </div>
            </div>
          ...
          <div class="space-y-4 pt-8">
            ...
            <!-- Entrega -->
            <div class="rounded-2xl bg-white ring-1 ring-black/10 shadow-[0_10px_28px_rgba(16,24,40,0.06)] overflow-hidden">
              <div class="px-5 py-3 border-b border-black/[0.06] flex items-center gap-2">
                ...
                <span class="text-[11px] font-bold tracking-widest text-gray-400 uppercase">Entrega</span>
              </div>
              <div class="px-5 py-4 grid grid-cols-2 gap-x-4 gap-y-4">
                <div class="col-span-2">
                  <p class="text-[11px] text-gray-400">Fecha</p>
                  <p class="mt-0.5 text-[18px] font-bold text-[#111827]">{{ formatDate(order.deliveryDate) }}</p>
                </div>
            ...
            <!-- Cliente -->
            <div v-if="order.customer" class="rounded-2xl bg-white ring-1 ring-black/10 shadow-[0_10px_28px_rgba(16,24,40,0.06)] overflow-hidden">
              <div class="px-5 py-3 border-b border-black/[0.06] flex items-center gap-2">
                ...
                <span class="text-[11px] font-bold tracking-widest text-gray-400 uppercase">Cliente</span>
              </div>
            ...
            <!-- Montaje -->
            <div v-if="order.setupPersonName" class="rounded-2xl bg-white ring-1 ring-black/10 shadow-[0_10px_28px_rgba(16,24,40,0.06)] px-5 py-4">
              <p class="text-[11px] font-bold tracking-widest text-gray-400 uppercase mb-1">Responsable de montaje</p>
      ...
      >
        ...
        <div class="absolute bottom-5 left-1/2 -translate-x-1/2 flex items-center gap-2">
          <button
            ...
            @click.stop="zoom = Math.max(1, zoom - 0.5); if (zoom === 1) pan = { x: 0, y: 0 }"
          >
            <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M5 12h14"/></svg>
          </button>
          <span class="text-white/70 text-[12px] font-semibold tabular-nums w-10 text-center">{{ Math.round(zoom * 100) }}%</span>
          <button
            class="rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-sm text-white p-2 transition"
          ...
          >
            <svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M4 4l16 16M20 4L4 20"/></svg>
          </button>
        </div>
        ...
        <!-- Hint primera vez -->
        <p class="absolute top-5 left-1/2 -translate-x-1/2 text-white/40 text-[11px] select-none pointer-events-none">
  ...
  <Teleport to="body">
    <Transition
      ...
      leave-to-class="opacity-0"
    >
      <div v-if="confirmOpen" class="fixed inset-0 z-50 flex items-center justify-center px-4">
        <!-- Backdrop -->
        <div class="absolute inset-0 bg-black/50 backdrop-blur-sm" @click="confirmOpen = false" />
        ...
        <div class="relative z-10 w-full max-w-[380px] rounded-3xl bg-white shadow-[0_24px_60px_rgba(0,0,0,0.18)] ring-1 ring-black/[0.08] overflow-hidden">
          ...
          <div class="flex flex-col items-center pt-8 pb-5 px-8 text-center">
            ...
            <p class="mt-1.5 text-[13px] text-gray-500 leading-relaxed">
              <span class="font-semibold text-[#111827]">{{ order?.orderCode }}</span> pasará de
            </p>
            <!-- Flecha de estado -->
            <div class="mt-3 flex items-center justify-center gap-2.5">
              ...
              <span class="inline-flex rounded-full px-3 py-1 text-[12px] font-bold bg-[#FFBEE6] text-[#C9007C]">{{ advanceNextLabel }}</span>
            </div>
          </div>
