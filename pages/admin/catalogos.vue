...
import ColorEditModal, { type ColorEditPayload } from '~/components/ColorEditModal.vue'
import {
  catalogsService,
  type ColorItem,
  type BreadTypeItem,
...
const fillingsPagination = ref({ limit: 10, offset: 0, totalPages: 1, currentPage: 1 })
...
const flavorsPagination = ref({ limit: 10, offset: 0, totalPages: 1, currentPage: 1 })
...
const frostingsPagination = ref({ limit: 10, offset: 0, totalPages: 1, currentPage: 1 })
...
const stylesPagination = ref({ limit: 10, offset: 0, totalPages: 1, currentPage: 1 })
...
const flowersPagination = ref({ limit: 10, offset: 0, totalPages: 1, currentPage: 1 })
...
type AnyItem = { id: string; name: string; description?: string; value?: string }
...
function openEdit(block: BlockKey, item: AnyItem) {
  ...
  editMode.value = 'edit'
  editBlock.value = block
  editModel.value = { id: item.id, name: item.name, description: item.description ?? '' }
  editOpen.value = true
}
  ...
  pendingAction.value = async () => {
    const handlers: Record<BlockKey, () => Promise<void>> = {
      ...
      cubierta: async () => {
        ...
        await loadFrostings()
        if (selected.value.cubierta === item.name) selected.value.cubierta = null
      },
...
async function onSaveColor(payload: ColorEditPayload) {
  ...
  await catalogsService.createColor({ name: payload.name, value: payload.value })
  await loadColors()
}
          ...
          <div class="max-h-[260px] space-y-1 overflow-auto p-2">
            ...
            <div v-if="colorsLoading" class="px-3 py-2 text-[13px] text-[#6B7280]">
              ...
              <button
                ...
                :class="selected[card.key as BlockKey] === it.name ? 'bg-[#F8D7E6]' : 'hover:bg-[#FCE4F2]'"
              ...
              <div v-if="card.key === 'relleno' && fillingsPagination.currentPage < fillingsPagination.totalPages" class="text-center mt-2">
                <button type="button" class="px-3 py-1 text-sm font-medium text-[#1F1F1F] bg-white border border-[#1F1F1F] rounded-md hover:bg-[#F6F6F7]" @click="loadMoreFillings">
                  Cargar más
                </button>
              </div>

              <div v-if="card.key === 'sabor' && flavorsPagination.currentPage < flavorsPagination.totalPages" class="text-center mt-2">
                <button type="button" class="px-3 py-1 text-sm font-medium text-[#1F1F1F] bg-white border border-[#1F1F1F] rounded-md hover:bg-[#F6F6F7]" @click="loadMoreFlavors">
                  Cargar más
                </button>
              </div>

              <div v-if="card.key === 'cubierta' && frostingsPagination.currentPage < frostingsPagination.totalPages" class="text-center mt-2">
                <button type="button" class="px-3 py-1 text-sm font-medium text-[#1F1F1F] bg-white border border-[#1F1F1F] rounded-md hover:bg-[#F6F6F7]" @click="loadMoreFrostings">
                  Cargar más
                </button>
              </div>

              <div v-if="card.key === 'estilo' && stylesPagination.currentPage < stylesPagination.totalPages" class="text-center mt-2">
                <button type="button" class="px-3 py-1 text-sm font-medium text-[#1F1F1F] bg-white border border-[#1F1F1F] rounded-md hover:bg-[#F6F6F7]" @click="loadMoreStyles">
                  Cargar más
                </button>
              </div>

              <div v-if="card.key === 'flor' && flowersPagination.currentPage < flowersPagination.totalPages" class="text-center mt-2">
                <button type="button" class="px-3 py-1 text-sm font-medium text-[#1F1F1F] bg-white border border-[#1F1F1F] rounded-md hover:bg-[#F6F6F7]" @click="loadMoreFlowers">
                  Cargar más
                </button>
              </div>
            </template>
            ...
            <template v-else>
              ...
              <button
                ...
                type="button"
                class="group flex w-full items-center justify-between gap-3 rounded-lg px-3 py-2 text-left text-[13px] text-[#2B2B2B] transition cursor-pointer"
                :class="selected.color === c.name ? 'bg-[#F8D7E6]' : 'hover:bg-[#FCE4F2]'"
              ...
              >
                ...
                <span class="flex items-center gap-3 shrink-0">
                  <span class="h-5 w-5 rounded-md border border-black/10 shadow-sm" :style="{ backgroundColor: c.value }" />
